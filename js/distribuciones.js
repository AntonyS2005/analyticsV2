let tamanoX = 600,
  tamanoy = 400,
  colorResultados = "#0ed700",
  colorNoResultados = "#076800";
...
document.getElementById("distribucion").addEventListener("change", function () {
  ...
  document.getElementById("binomial-params").style.display =
    distribucion === "binomial" ? "block" : "none";
  document.getElementById("poisson-params").style.display =
    distribucion === "poisson" ? "block" : "none";
  document.getElementById("normal-params").style.display =
    distribucion === "normal" ? "block" : "none";
});
...
function graficar() {
  ...
  if (distribucion === "binomial") {
    ...
    xValues = Array.from(
      { length: numPuntos },
      (_, i) => mu - 3 * sigma + (i * (6 * sigma)) / (numPuntos - 1)
    );
    ...
    const trace = {
      ...
      marker: {
        color: xBarra.map((x) =>
          x + anchoBarra / 2 >= resultadoIndice - anchoBarra / 2 &&
          x + anchoBarra / 2 < resultadoIndice + anchoBarra / 2
            ? colorResultados
            : colorNoResultados
        ),
  ...
  const trace = {
    ...
    marker: {
      color: xValues.map((x) =>
        x === resultadoIndice ? colorResultados : colorNoResultados
      ),
      line: {
        width: 1,
        color: "black",
