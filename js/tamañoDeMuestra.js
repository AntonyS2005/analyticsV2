  ...
  tipoDeMuestra.addEventListener("change", function () {
    ...
    if (inputsConfig[opcion]) {
      for (let i = 0; i < inputsConfig[opcion].length; i += 2) {
        let inputDiv = crearInput(
          inputsConfig[opcion][i],
          inputsConfig[opcion][i + 1]
        );
  ...
  function calcEstProConN(nc, n, p, d, s, opcion) {
    ...
    switch (opcion) {
      case "proConPo":
        ...
        result =
          (n * Math.pow(za, 2) * p * q) /
          (Math.pow(d, 2) * (n - 1) + Math.pow(za, 2) * p * q);
      ...
      case "medConPo":
        ...
        za = calZa(nc, ns);
        result =
          (n * Math.pow(za, 2) * Math.pow(s, 2)) /
          (Math.pow(d, 2) * (n - 1) + Math.pow(za, 2) * Math.pow(s, 2));
        return { ns, za, result: Math.ceil(result) };
      default:
