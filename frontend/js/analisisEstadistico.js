...
function generarTablaPorIntervalos(dato) {
  ...
  const cabeceras = ['LI', 'LS', 'XI', 'F', 'Fr%', 'Fa', 'Fa%', 'fd', 'fd%', 'F*Xi', 'd', 'f*|d|', 'F*D^2', 'F*D^3', 'F*D^4'];
  ...
  const encabezadosDatos = ["Moda", "Mediana", "Media Aritmética", "Desviación Media", "Desviación Estándar", "Skewness (SK)", "Kurtosis (K)"];
  const datosResumen = [moda, mediana, mediaArit, desviacionMed, desviacionEst, sk, k];
...
document.getElementById('btnCalcularMedida').addEventListener('click', function() {
const tipo = document.getElementById('tipoMedida').value; // 'cuartil', 'decil' o 'percentil'
let indiceIngresado = parseInt(document.getElementById('indiceMedida').value, 10);
if (isNaN(indiceIngresado) || indiceIngresado < 1) {
  document.getElementById('resultadoMedida').textContent = 'Por favor ingresa un número válido (mayor o igual a 1).';
  return;
}
...
if (medida !== null) {
  document.getElementById('resultadoMedida').textContent = `La medida solicitada es: ${medida}`;
} else {
  document.getElementById('resultadoMedida').textContent = 'El número ingresado excede la cantidad de medidas disponibles para el tipo seleccionado.';
}
});
...
function analizarDatosSimples(datos) {
  ...
  let valoresUnicos = Object.keys(conteo).map(Number).sort((a, b) => a - b);
  ...
  function obtenerValorPorPorcentaje(porcentaje) {
    ...
    if (piso === techo) {
      ...
      return datosOrdenados[piso] + dif * (datosOrdenados[techo] - datosOrdenados[piso]);
    }
  }
...
function generarTablaSimple(datos) {
  ...
  const encabezados = ["x", "f", "fr%", "Fa", "Fa%", "Fd", "Fd%", "f*x", "d", "f*d", "f*|d|", "f*d^2", "f*d^3", "f*d^4"];
